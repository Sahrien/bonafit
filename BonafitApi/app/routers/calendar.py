...
from app.deps import actor_of, require_admin, require_not_must_change, utcnow
...
def list_trainer_schedules(
    ...
    if trainerId:
        query = query.where(TrainerSchedule.trainer_id == trainerId)
...
def list_appointments(
    ...
    if user.role == "client":
        ...
        query = query.where(Appointment.client_id == clientId)
    if trainerId:
        query = query.where(Appointment.trainer_id == trainerId)
    if from_:
...
def get_availability(
    ...
    if service is None:
        raise NotFoundError("service", serviceId)
